...
# 需要 brglm2；若沒裝會自動退回 glm.fit
.mstep_firth <- function(X_aug, y, w, start = NULL, offset = NULL, maxit = 50) {
  # X_aug 必須含截距欄（1）；若你已經 cbind(1, ...) 就 OK
  if (requireNamespace("brglm2", quietly = TRUE)) {
    ctrl <- brglm2::brglmControl(maxit = maxit, method = "brglmFit")
    fit  <- try(
      brglm2::brglm.fit(
        x = X_aug, y = y, weights = w, offset = offset,
        family = binomial("logit"),
        type   = "AS_mean",            # <-- Firth / Jeffreys（mean bias reduction）
        control = ctrl, start = start
      ),
      silent = TRUE
    )
    if (!inherits(fit, "try-error") && all(is.finite(coef(fit)))) {
      return(as.numeric(coef(fit)))
    }
  }
  # 失敗或沒裝 brglm2 時安全回退
  as.numeric(coef(glm.fit(
    x = X_aug, y = y, weights = w, offset = offset,
    family = binomial(),
    start  = start,
    control = glm.control(maxit = maxit, epsilon = 1e-8)
  )))
}
...
hetero_em_logit <- function(
  ...
  for (it in seq_len(maxit)) {
    ...
    # ---- M-step：Firth/Jeffreys（AS_mean）----
    X_aug  <- cbind(1, Xs)
    beta_m <- .mstep_firth(X_aug, ys, w_vec, start = beta_old, offset = NULL, maxit = 50)
